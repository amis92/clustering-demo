...
public class Point {
	 ...
	 * @throws Exception
	 */
	public double findClosestNeighbors(ArrayList<Point> points, int k)
			throws Exception {
	...
	public Type calculateClass(double maxDensity) {
		double minNeighborDensity = Double.MAX_VALUE, maxNeighborDensity = -Double.MAX_VALUE;
		...
		if (relativeDensity > maxNeighborDensity
				&& this.relativeDensity > 0.5d * maxDensity)
			type = Type.CSO;
