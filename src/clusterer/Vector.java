...
public class Vector {
	...
	public static double euclideanDistance(Vector first, Vector second) {
		...
		for (int i = 0; i < first.values.length; i++) {
			sumOfSquares += (first.values[i] - second.values[i])
	...
	public static Vector calculateCenter(Vector... vectors) {
		...
		Vector center = new Vector(Arrays.copyOf(vectors[0].values,
				vectors[0].values.length));
		for (int i = 1; i < vectors.length; i++) {
			if (vectors[i].values.length != center.values.length)
				throw new IllegalArgumentException("vector " + i
						+ "'s dimension is not compatible with first vector!");
